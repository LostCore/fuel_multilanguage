	...
	public function before_insert(Model $obj)
	{
		...
		foreach ($properties as $property)
		{
            if(is_array($obj->{$property})){
                $source .= '-'.$obj->{$property[0]}; //se è multilingua, allo 0 c'è la lingua di default
            }elseif(is_serialized($obj->{$property})){
                $uns = unserialize($obj->{$property});
                $source .= '-'.$uns[0]; //se è multilingua, allo 0 c'è la lingua di default
            }else{
                $source .= '-'.$obj->{$property};
            }
		}
